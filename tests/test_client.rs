...
async fn test_coingecko_healthcheck() {
    ...
    assert_eq!(response.status().is_success(), true, "CoinGecko API is down");

    let body = response.text().await.unwrap();
    
    // Ensure that the response contains the expected data (detail_platforms)
    assert!(body.contains("detail_platforms"), "Response body doesn't contain expected data");
    
    // Ensure that the response contains the expected platform address for SUI
    assert!(body.contains("0x2::sui::SUI"), "Response body doesn't contain expected data");
}
...
async fn test_coinllama_healthcheck() {
    ...
    // Check that the response status code is 200 OK (successful)
    assert_eq!(response.status().is_success(), true, "CoinLlama API is down");

    // Get the response body as text
    let body = response.text().await.unwrap();
    
    // Ensure that the response contains the expected data for SUI
    assert!(body.contains("SUI"), "Response body doesn't contain expected data");
}
