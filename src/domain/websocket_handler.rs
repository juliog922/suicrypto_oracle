...
use crate::{
    AppError, 
    infraestructure::api_client::ApiClient
};
...
impl WebSocketHandler {
    ...
    pub async fn connect(&self) -> Result<(), AppError> {
        ...
        let server_host = match env::var("SERVER_HOST") {
            Ok(host) => host,
            Err(_) => {
                return Err(AppError::FileError("SERVER_HOST not found in .env file".to_string()));
            }
        };
        ...
        let (ws_stream, _) = connect_async(&server_url)
            .await
            .map_err(|e| AppError::TcpError(format!("Error connecting to {}: {}", &server_url, e)))?;
        ...
        while let Some(Ok(msg)) = read.next().await {
            if let Message::Text(text) = msg {
                ...
                if text == "REQUEST_TOKEN_PRICE" {
                    match ApiClient::new(self.contract_address.clone()).fetch_price().await {
                        Ok(api_response) => {
                            match ApiClient::process_api_response(&api_response) {
                                Ok(processed_response) => {
                                    ...
                                    write
                                        ...
                                        .await
                                        .map_err(|e| AppError::WebSocketMessageError(format!("Error sending message: {}", e)))?;
                                    debug!("Message sent successfully");
                                }
                                Err(e) => {
                                    // Handle API response processing error
                                    write
                                        .send(Message::Text(format!("Error processing response: {}", e)))
                                        .await
                                        .map_err(|e| AppError::WebSocketMessageError(format!("Error sending error message: {}", e)))?;
                        ...
                        Err(e) => {
                            ...
                            write
                                ...
                                .await
                                .map_err(|e| AppError::WebSocketMessageError(format!("Error sending API error: {}", e)))?;
